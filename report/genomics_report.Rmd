...
acmg_genes <- c("BRCA1", "BRCA2", "TP53", "STK11", "MLH1", "MSH2", "MSH6", "PMS2", "APC", "MUTYH", "VHL", "MEN1", "RET", "PTEN", "RB1", "SDHD", "SDHAF2", "SDHC", "SDHB", "TSC1", "TSC2", "WT1", "NF2")

# Function to filter VCF to ACMG genes and identify rare/clinvar/LoF variants
filter_acmg_variants <- function(vcf_file) {
  vcf <- readVcf(vcf_file, "hg19")
  
  # Filter for ACMG genes
  acmg_variants <- vcf[seqnames(vcf) %in% acmg_genes]
  
  # Annotate variants using Ensembl VEP
  vep_results <- ensemblVEP::vep(acmg_variants)
  
  # Filter for rare variants (AF < 0.01)
  rare_variants <- vep_results[vep_results$AF < 0.01, ]
  
  # Filter for ClinVar pathogenic variants
  clinvar_variants <- rare_variants[grepl("pathogenic", rare_variants$CLIN_SIG, ignore.case = TRUE), ]
  
  # Filter for loss of function variants
  lof_variants <- rare_variants[grepl("stop_gained|frameshift_variant|splice_acceptor_variant|splice_donor_variant", rare_variants$Consequence, ignore.case = TRUE), ]
  
  # Combine all filtered variants
  filtered_variants <- rbind(clinvar_variants, lof_variants)
  
  # Extract relevant information
  variant_table <- data.frame(
    Chromosome = seqnames(filtered_variants),
    Position = start(filtered_variants),
    Reference = ref(filtered_variants),
    Alternate = alt(filtered_variants),
    Gene = filtered_variants$SYMBOL,
    HGVS = filtered_variants$HGVSc,
    Classification = filtered_variants$CLIN_SIG,
    Disease = filtered_variants$Phenotype,
    Literature = paste0("https://www.ncbi.nlm.nih.gov/pubmed/?term=", filtered_variants$SYMBOL)
  )
...
# Filter the VCF file and output the table
acmg_variant_table <- filter_acmg_variants(vcf_file)
acmg_variant_table
