...
#b:4c456a7a-e189-11ef-bb21-002436b6d0cf
...
setwd(workingdir)
vcf_file <- "391b3394-fb01-11ef-9450-002436b6d0cf_subset.tab"
...
X250204_195258_export_variant$reval_score<-X250204_195258_export_variant$`Score...96`
...
variant_high <- acmg_var %>%
  ...
  distinct(Gene,cDNA_change,Protein_Change, AF,DP,reval_score,`Rank_Score (no MaxAF)`,alpha_score, Clinical_Significance,Disease_Names) %>%
  arrange(-reval_score)
...
#  filter(aar_gene_var$Allele_Origin=='germline') %>%
  ...
  filter(!(grepl('enign', Clinical_Significance))) %>%
  filter(Sequence_Ontology!="synonymous_variant") %>%
  distinct(Gene,cDNA_change,Protein_Change, AF,DP,reval_score,`Rank_Score (no MaxAF)`,alpha_score, Condition,Outcome,Clinical_Significance) %>%
  arrange(-reval_score, Clinical_Significance)
...
wellness_var<-wellness_var_raw %>%
  dplyr::select(Gene,Sequence_Ontology,cDNA_change,Protein_Change,alpha_score,`Rank_Score (no MaxAF)`,`Allele_Frequencies TGP`,Chrom...2,VCF_Position,`VCF_Ref Allele`,`VCF_Alt Allele`,AF,DP, reval_score,type)
