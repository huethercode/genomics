...
#palm:21a36b0e-e5ae-11ef-9dc1-002436b6d0cf
#bob:4c456a7a-e189-11ef-bb21-002436b6d0cf
#ir: 391b3394-fb01-11ef-9450-002436b6d0cf
workingdir<-"~/Documents/genomics/output/391b3394-fb01-11ef-9450-002436b6d0cf"
setwd(workingdir)
vcf_file <- "391b3394-fb01-11ef-9450-002436b6d0cf.vcf"
...
mean_dp <- mean(depth_df$DP, na.rm = TRUE)
sd_dp <- sd(depth_df$DP, na.rm = TRUE)

# Normalize depth values
depth_df$Norm_DP <- (depth_df$DP - min(depth_df$DP, na.rm = TRUE)) / 
                     (max(depth_df$DP, na.rm = TRUE) - min(depth_df$DP, na.rm = TRUE))

# Assign colors based on depth thresholds
depth_df$Color <- ifelse(depth_df$DP >= mean_dp + 3 * sd_dp, "red", 
                  ifelse(depth_df$DP <= mean_dp - 3 * sd_dp, "blue", "black"))
...
kpPoints(kp, chr = depth_df$Chr, x = depth_df$Position, y = depth_df$Norm_DP, 
         ymin = 0.2, ymax = 1, r0 = 0.05, r1 = 0.75, col = depth_df$Color, lwd = 2)
```
...
JOIN genes_df df2
ON df1.Chr = df2.chromosome
AND df1.Position BETWEEN df2.Start_1 AND df2.Stop_1
"
...
df$AF<-ifelse(df$AD>0,df$AD/df$DP,1)

# Plot allele frequency distribution
p_af<-ggplot(df, aes(x = AF)) +
...
variant_high <- acmg_var %>%
  ...
  distinct(Gene,cDNA_change,Protein_Change, AF,DP,reval_score,`Rank_Score (no MaxAF)`,`Score...14`, Clinical_Significance,Chrom...1,Position,Ref_Base,Alt_Base) %>%
  arrange(-reval_score)
...
aar_gene_var_out <- aar_gene_var %>%
  ...
  filter(!(grepl('enign', Clinical_Significance))) %>%
  filter(Sequence_Ontology!="synonymous_variant") %>%
  distinct(Gene,cDNA_change,Protein_Change, AF,DP,reval_score,`Rank_Score (no MaxAF)`,`Score...14`, Condition,Outcome,Clinical_Significance) %>%
  arrange(-reval_score)
...
wellness_var<-wellness_var_raw %>%
  dplyr::select(Gene,Sequence_Ontology,cDNA_change,Protein_Change,Score...14,`Rank_Score (no MaxAF)`,`Allele_Frequencies TGP`,Chrom...1,VCF_Position,`VCF_Ref Allele`,`VCF_Alt Allele`,AF,DP, reval_score,type)
...
wellness_var<-wellness_var %>% mutate(impact=case_when(
  reval_score >=0.75 | `Rank_Score (no MaxAF)`>=.75 | `Score...14` >.75 ~ 1,
  reval_score >=0.5 | `Rank_Score (no MaxAF)`>=.5 | `Score...14` >.5 ~ 2, 
  is.na(reval_score) | is.na(`Rank_Score (no MaxAF)`) | is.na(`Score...14` >.5) ~ 4,   
  TRUE ~ 3)
            ) %>%
  group_by(type) %>%
...
# **Overall Summary**  
This genomic report provides an in-depth analysis of **genetic variants, ancestry, pharmacogenomics (PGx), hereditary conditions, wellness traits, and polygenic risk scores (PRS)**. The data is derived from a **high-quality VCF file**, confirming that sequencing depth, genotype quality, and variant classification meet **PASS** thresholds. Key findings include **actionable hereditary variants, wellness trait markers, and polygenic risk factors** for various health conditions.

### **Key Takeaways:**
1. **VCF Quality Control:** The dataset meets sequencing depth and genotype quality requirements, ensuring reliable variant interpretation.
2. **Hereditary Risk Genes:** Variants in **RET (p.Gly691Ser)** show conflicting pathogenicity but are linked to **Multiple Endocrine Neoplasia Type IIA and Medullary Thyroid Cancer**.
3. **Wellness Traits:** Identified **genetic predispositions** for **exercise response, metabolic health, sleep patterns, and aging**.
4. **Polygenic Risk Scores:** Higher risk for **blood protein levels, vitamin D deficiency, and smoking behavior**.

---

## **Key Insights Table**
| **Category**            | **Key Finding** | **Potential Impact** | **Actionable Recommendations** |
|-------------------------|----------------|----------------------|--------------------------------|
| **Hereditary Conditions** | **RET (p.Gly691Ser)** | Potential risk for **thyroid cancer** | Consult an **endocrinologist** for genetic counseling and periodic screenings. |
| **Wellness: Fat Loss & Exercise** | **ADRB2, LPL** | Low response to **cardio-based fat loss** | Increase resistance training and focus on **HIIT** over steady-state cardio. |
| **Wellness: Mental Acuity** | **APOE, BDNF** | Cognitive performance and **brain aging** | Prioritize **omega-3s, cognitive training, and quality sleep**. |
| **Wellness: Lactose Intolerance** | **MCM6 variant detected** | Reduced lactase enzyme activity | Reduce dairy intake or opt for **lactose-free alternatives**. |
| **Polygenic Risk: Vitamin D Deficiency** | **Serum 25-Hydroxyvitamin D (-1.275 PRS score)** | High risk for **low vitamin D levels** | Supplement with **2000 IU/day vitamin D3** and get **more sunlight** exposure. |
| **Polygenic Risk: Smoking Behavior** | **High-risk genetic predisposition** | Higher tendency for **nicotine addiction** | Avoid smoking/vaping; consider **behavioral therapy** for cessation. |

---

## **Lifestyle Action Plan**
### **1. Cancer & Endocrine Health**
- **Action:** Schedule a consultation with a **genetic counselor** to discuss **RET variant risks**.
- **Why:** RET mutations are linked to **thyroid and adrenal gland cancers**.
- **Expected Outcome:** Early detection and **preventive screenings**.

### **2. Cardiovascular & Metabolic Health**
- **Action:** Switch to a **high-intensity training (HIIT)** routine, as **ADRB2, LPL variants** indicate a weaker fat loss response to steady-state cardio.
- **Why:** **HIIT improves metabolism and fat oxidation** more effectively for those with these variants.
- **Expected Outcome:** Improved **fat loss and cardiovascular fitness**.

### **3. Cognitive & Mental Health**
- **Action:** **Increase omega-3 fatty acid intake** (fatty fish, walnuts, flaxseeds) and engage in cognitive training apps like **Lumosity**.
- **Why:** APOE, BDNF variants suggest a **higher risk for age-related cognitive decline**.
- **Expected Outcome:** **Better brain function and memory retention**.

### **4. Bone & Vitamin D Health**
- **Action:** **Supplement with vitamin D3 (2000 IU/day)** and increase outdoor sun exposure.
- **Why:** PRS indicates a **high risk for vitamin D deficiency**.
- **Expected Outcome:** Improved **bone density and immune function**.

### **5. Lifestyle & Behavioral Health**
- **Action:** If at risk for **nicotine addiction**, avoid smoking and consider **counseling or nicotine replacement therapy (NRT)**.
- **Why:** Genetic predisposition to smoking behavior increases **addiction potential**.
- **Expected Outcome:** **Lower risk of nicotine dependence** and associated diseases.
...
- Gene curation coalition [GenCC](https://thegencc.org/)n D & Bone Health:** [NIH Vitamin D Fact Sheet](https://ods.od.nih.gov/factsheets/VitaminD-HealthProfessional/)
